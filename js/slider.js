class Slider{
  ...
  #setup(){
    this.$slWrapper.classList.add('slick__slider__container')
    this.$slider.classList.add('slick__slider')
    const slides = [...this.$slider.children]
    slides.forEach(el => el.classList.add('slick__slide'))
  ...
  #render(){
    ...
    const dots = [...this.$slWrapper.querySelectorAll('.slick__slider__dots li')]
    dots.forEach(dt => {
  ...
  #getButtons(){
    const buttons = `
      <div class="slick__slider__buttons">
        <button class="slick__slider__prev">&#10094;</button>
        <button class="slick__slider__next">&#10095;</button>
      </div>
    `
    this.$slWrapper.insertAdjacentHTML('beforeend', buttons)
    this.prevSL = this.prevSL.bind(this)
    this.nextSL = this.nextSL.bind(this)
    const prevBtn = this.$slWrapper.querySelector('.slick__slider__prev')
    const nextBtn = this.$slWrapper.querySelector('.slick__slider__next')
  ...
  #getDots(){
    ...
    dotsBlock.classList.add('slick__slider__dots')
